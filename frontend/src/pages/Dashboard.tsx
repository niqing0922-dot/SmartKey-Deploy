...
export function DashboardPage() {
  ...
  return (
    <div id="page-dashboard" className="page page-active">
      ...
      <div className="page-body linear-workbench dashboard-linear-workbench">
        <section className="linear-left">
          <div className="linear-panel-title">{language === 'zh' ? '关键指标' : 'Key Metrics'}</div>
          <div className="linear-metric-list">
            <div><span>{copy.total}</span><strong>{stats?.keywords.total ?? 0}</strong></div>
            <div><span>{copy.pending}</span><strong>{stats?.keywords.pending ?? 0}</strong></div>
            <div><span>{copy.planned}</span><strong>{stats?.keywords.planned ?? 0}</strong></div>
            <div><span>{copy.done}</span><strong>{stats?.keywords.done ?? 0}</strong></div>
            <div><span>{copy.coverage}</span><strong>{stats?.keywords.coverage ?? 0}%</strong></div>
          </div>
            ...
            <table className="tbl linear-table">
              ...
              <tbody>
                {!stats?.recent_articles.length ? (
                  <tr><td colSpan={3}><div className="empty">{copy.emptyArticles}</div></td></tr>
            ...
            <table className="tbl linear-table">
              ...
              <tbody>
                {!stats?.pending_keywords.length ? (
                  <tr><td colSpan={4}><div className="empty">{copy.emptyPending}</div></td></tr>
