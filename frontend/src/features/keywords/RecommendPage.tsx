...
export function RecommendPage() {
  ...
  useEffect(() => {
    const draft = consumeWorkbenchTaskDraft('keywords.recommend')
    ...
    <div id="page-recommend" className="page page-active">
      ...
      <div className="page-body linear-workbench ai-linear-workbench">
        <section className="linear-left">
          ...
          <div className="linear-inspector-grid">
            ...
            <div className="field-block">
              ...
              <select value={count} onChange={(event) => setCount(event.target.value)}>
                <option value="8">{formatCount('8')}</option>
                <option value="12">{formatCount('12')}</option>
                <option value="15">{formatCount('15')}</option>
                <option value="20">{formatCount('20')}</option>
                <option value="30">{formatCount('30')}</option>
                <option value="50">{formatCount('50')}</option>
                <option value="100">{formatCount('100')}</option>
              </select>
            </div>
            <div className="field-block">
              ...
              <div className="aw-section-picker">
                {typeOptions.map((type) => (
                  <span
                    key={type}
                    className={`chip ${hasSelection.has(type) ? 'sel' : ''}`}
          ...
          <button id="btn-rec" className="btn btn-primary btn-full" onClick={submit} disabled={loading}>
            ...
            <table className="tbl linear-table">
              ...
              <tbody>
                {!items.length ? (
                  <tr>
                    <td colSpan={4}>
                      <div className="empty">{copy.resultTitle}</div>
                    </td>
                  </tr>
