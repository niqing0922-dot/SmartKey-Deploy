...
import { cloudApi, localDataApi } from '@/services/api'
...
export function LocalDataPage() {
  ...
  const importSnapshotToCloud = async () => {
    if (!importText.trim()) return
    const parsed = JSON.parse(importText)
    const result = await cloudApi.importSnapshot(parsed)
    setMessage(`Cloud import complete. Keywords: ${result.imported.keywords || 0}, articles: ${result.imported.articles || 0}, GEO drafts: ${result.imported.geo_article_drafts || 0}.`)
    setImportText('')
  }
  ...
  return (
    <div id="page-local-data" className="page page-active">
      ...
      <div className="page-body local-data-grid">
        ...
        <div className="card"><div className="card-header"><div className="card-title">{copy.importSnapshot}</div></div><label>{copy.pasteJson}</label><textarea data-testid="local-data.import-textarea" rows={10} value={importText} onChange={(event) => setImportText(event.target.value)}></textarea><div className="article-panel-actions"><button className="btn btn-primary btn-sm" data-testid="local-data.cloud-import-button" onClick={importSnapshotToCloud}>Import to Cloud Workspace</button><button className="btn btn-sm" data-testid="local-data.import-button" onClick={importSnapshot}>{copy.importBtn}</button></div></div>
